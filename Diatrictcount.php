...
<tr bgcolor="#FFFFCC">
    ...
    <td width="191" align="center">SEATS</td>
</tr>
...
if(isset($_POST['sub'])){
	...
	$query=mysql_query("SELECT E_party,count(vote_count) as cnt, TRUNCATE((count(vote_count) / (SELECT count(indexv) FROM vote)) * 100,2) as pcnt FROM vote WHERE distric='$district' AND province='$province' AND vote_count=1 GROUP BY E_party ORDER BY count(vote_count) DESC");
	...
	echo "<td bgcolor=\"#EBEBEB\" align=\"center\">".floor($row['pcnt']/$row['cnt'])."</td>";
	echo "</tr>";	
	}
...
$values=array();
	...
	$query=mysql_query("SELECT E_party,count(vote_count) as cnt FROM vote WHERE distric='$district' AND province='$province' AND vote_count=1 GROUP BY E_party");
	while($row=mysql_fetch_array($query)){
...
<table width="300" border="0" align="center">
	...
	<tr><td><font color="#990000" size="+1">Summery</font> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="grp3.php?read=ok"><font color="#FF9900">Graphical view</font></a></td></tr>
...
if(isset($_POST['sub'])){
	...
	$query2=mysql_query("SELECT count(vote_count) as cunt FROM vote WHERE distric='$district' AND province='$province' AND vote_count=1");
	while($row=mysql_fetch_array($query2)){
		$cunt=$row['cunt'];
	}
	
	$query3=mysql_query("SELECT count(rejectVote) as rcunt FROM vote WHERE distric='$district' AND province='$province' AND rejectVote=1");
	while($row=mysql_fetch_array($query3)){
		$rcunt=$row['rcunt'];
	}
	
$query4=mysql_query("SELECT count(indexv) as ttl FROM vote WHERE distric='$district' AND province='$province'");
...
//seat calculation-----------------------------------------------------------------------------------------------------------------
	...
	$queryseat=mysql_query("SELECT * FROM seats WHERE province='$province' AND distric='$district'");
	while($row=mysql_fetch_array($queryseat)){
	
	$dseat=$row['seats'];
		
	}
	
	$voteforseat=$cunt/$dseat;
	
	$querymaxvote=mysql_query("SELECT E_party,count(vote_count) as cnt FROM vote WHERE province='$province' AND distric='$district' GROUP BY E_party");
	while($row=mysql_fetch_array($querymaxvote)){
	
		$maxcount=$row['cnt'];
		
	}
	
	$seats=$maxcount/$voteforseat;
